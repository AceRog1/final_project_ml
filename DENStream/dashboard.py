...
# # Importaciones para el cálculo de métricas
...
# from ml_pipeline import FlightMLPipeline
...
#                 try:
#                     # Usar tiempo de modificación del archivo
...
#                         if features and self.processor.validate_features(features):
#                             features['timestamp'] = file_time.strftime('%Y%m%d%H%M%S')
#                             features['longitude'] = data.get('longitude')
#                             features['latitude'] = data.get('latitude')
#                             features['callsign'] = data.get('callsign', 'Unknown')
#                             features['icao24'] = data.get('icao24', 'Unknown')
#                             features['cluster'] = data.get('cluster')
#                             flight_data.append(features)
#                     else:
#                         file_path.unlink()
...
#             return pd.DataFrame(flight_data) if flight_data else pd.DataFrame()
...
#         # Usar posición actual si está disponible
...
#         # Agregar marcadores con colores por cluster
...
#                     <b>Velocidad:</b> {row['vel']:.1f} kt<br>
#                     <b>Altitud:</b> {row['alt']:.0f} ft<br>
...
#         # Gráfico de learning rate con decay
...
#         if df.empty:
#             return [None] * 8  # Retornar None para todos los gráficos

#         # 1. Distribución de velocidades (existente)
...
#         # 2. Distribución de altitudes (existente)
...
#         # 3. Scatter plot velocidad vs altitud (existente)
...
#         # 4. Distribución de direcciones (nuevo)
...
#         # 5. Rosa de los vientos (versión mejorada)
#         fig_windrose = None
#         try:
...
#             # Contar vuelos por bin de dirección y cluster
#             direction_counts = windrose_df.groupby(
#                 ['heading_bin', 'cluster_grouped']
#             ).size().reset_index(name='count')

#             # Crear gráfico polar robusto
...
#         # 7. Análisis temporal (nuevo)
#         fig_temporal = None
#         try:
#             df['datetime'] = pd.to_datetime(df['timestamp'], format='%Y%m%d%H%M%S', errors='coerce')
#             df = df.dropna(subset=['datetime'])
#             time_group = df.groupby(pd.Grouper(key='datetime', freq='15T')).size().reset_index(name='count')
#             fig_temporal = px.line(
#                 time_group,
#                 x='datetime',
#                 y='count',
#                 title='Flujo de Vuelos a lo Largo del Tiempo',
#                 labels={'datetime': 'Hora', 'count': 'Vuelos por 15 min'}
#             )
#         except Exception as e:
#             print(f"Error creando análisis temporal: {e}")
#             fig_temporal = None

#         # 8. Visualización 3D (nuevo)
...
#         # 3. Cohesión intra-cluster (distancia promedio al centroide)
#         intra_cluster_dists = []
...
#         # 4. Silhouette Score (requiere características numéricas)
#         silhouette = None
#         try:
...
#         # 5. Puntos outliers (cluster -1 o similar)
...
#         # 3. Visualización de clusters en espacio 2D
...
#         # 4. Silhouette Visual (si hay suficientes datos)
#         fig_silhouette = None
#         try:
...
#                     # Agregar los valores de silhouette para el i-ésimo cluster
...
#                     # Etiquetar los gráficos de silhouette con sus números de cluster en el medio
...
#                 # Línea vertical para el score promedio de silhouette
...
class FlightDashboard:
    ...
    def create_flight_analytics(self, df: pd.DataFrame):
        ...
        try:
            ...
            df = df.dropna(subset=['datetime'])
            #time_group = df.groupby(pd.Grouper(key='datetime', freq='15T')).size().reset_index(name='count')
            #time_group = df.groupby(pd.Grouper(key='datetime', freq='15T'), observed=False).size().reset_index(name='count')
            time_group = df.groupby(pd.Grouper(key='datetime', freq='5min'), observed=False).size().reset_index(name='count')
            fig_temporal = px.line(
                time_group,
                x='datetime',
                y='count',
